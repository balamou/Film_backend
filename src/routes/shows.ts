...
router.get("/shows/:start/:quantity/:lang", (req, res, next) => {
  ...
  Series.findAll().then(result => {
    const showsData = result.map(show => {
      return { id: show.id, posterURL: show.poster?.replace(/public\//,'') };
    });
...
const generateData = (quantity: number) => {
  const posters = [
    "https://cdn.shopify.com/s/files/1/0191/7850/products/RICKMORTY_39_-_COVER_A_FNL_WEB_1024x.jpg?v=1530034748",
